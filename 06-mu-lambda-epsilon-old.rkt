...
(define (comp syntax)
  (match syntax
    ...
    [(stx-expand macro argument)
      ...
      (define argument-value (val-expand (comp argument) context))
      (val-eps context (val-expand macro-value argument-value))]
...
(define (comp-sexpr sexpr)
  (val-expand (comp (parse-sexpr sexpr)) default-environment))
...
(test-case
  ...
  (define epsarg_x (val-epsargument))
  (define epsarg_y (val-epsargument))
  (check-true (set-member? (find-used-var-set (dag-context) (val-apply epsarg_x epsarg_x)) epsarg_x))
  (check-true (set-member? (find-used-var-set (dag-context) (val-apply epsarg_x epsarg_y)) epsarg_x))
  (check-true (set-member? (find-used-var-set (dag-context) (val-apply epsarg_x epsarg_y)) epsarg_y))
)
...
(define (reduce-sexpr environment sexpr)
  (reduce (dag-context) (val-expand (comp (parse-sexpr sexpr)) environment)))

;; reduce-def-sexpr :: SExpr -> Value
(define (reduce-def-sexpr sexpr)
  (reduce-sexpr default-environment sexpr))

(graph-to-svg-file (parse-sexpr '(negate 1)) "negate-syntax.svg")
(graph-to-svg-file (comp-sexpr '(negate 1)) "negate-compiled.svg")
(graph-to-svg-file (reduce-once-def-sexpr '(negate 1)) "negate-reduced.svg")
