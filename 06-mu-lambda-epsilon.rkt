...
(struct stx-eps (argument body) #:transparent)
...
(define (parse-sexpr sexpr)
  (cond
    ...
    [(list? sexpr) [match sexpr
      ...
      ([list 'eps arguments body] (foldr stx-eps (parse-sexpr body) arguments))
...
(struct val-symbol (value) #:transparent)
...
(define (environment-with-values-expansion environment symbols-with-values)
  (match symbols-with-values
    ...
    [(list-rest (list symbol value) rest)
      (environment-with-values-expansion (val-environment-child environment (val-symbol symbol) value) rest)]))
...
(test-case
  ...
  [check-equal?
    ...
    (val-environment-child (val-environment-empty) (val-symbol 'true) (val-true))]
  [check-equal?
    (environment-with-values (val-environment-empty) [(true (val-true)) (false (val-false))])
    (val-environment-child (val-environment-child (val-environment-empty) (val-symbol 'true) (val-true)) (val-symbol 'false) (val-false))]
)

;; lookup-valid-symbol :: Environment, Symbol -> Value. Error when not found.
(define (lookup-valid-symbol environment symbol)
  (match environment
    [(val-environment-empty) (error "Unbound symbol during compilation " symbol)]
    [(val-environment-child parent env-symbol value)
      (if (equal? env-symbol symbol)
        value
...
(test-case
  "lookup-valid-symbol"
  (check-equal? (lookup-valid-symbol (val-environment-child (val-environment-empty) (val-symbol 'x) (val-integer 2)) (val-symbol 'x)) (val-integer 2))
)
...
#|
    [(stx-mu argument body)
      ...
      (val-mu argument-value closure-body)]
    [(stx-let name value body)
      (define child-env (val-environment-child environment (val-symbol name) (comp environment value)))
      (comp child-env body)]
...
(test-case
  ...
  (define muarg_r (val-muargument))
  [check-equal?
    (comp-def-sexpr '(mu (x) x))
    (val-mu muarg_r muarg_r)]

  (define prim-+ (lookup-valid-symbol default-environment (val-symbol '+)))
  [check-equal?
    (comp-def-sexpr '(+ 2 3))
